...
describe("NFTMarket", () => {
  ...
  before(async () => {
    ...
    const NFTMarket = await ethers.getContractFactory("CryptogramMarket");
  ...
  describe("buyNFT", () => {
    ...
    it("should transfer ownership to the buyer and send the price to the seller", async () => {
      ...
      const receipt = await transaction.wait();
      // 99.9% of the price was added to the seller balance
      await new Promise((r) => setTimeout(r, 100));
      ...
      // 0.01% of the price was kept in the contract balance
